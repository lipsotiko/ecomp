...
where review_section = 'QUALITY' and le.lead_element_id = @ELEMENT_ID
...
left join ce_supervisor_dashboard_user super_ce
on ce.user_id = super_ce.claims_examiner_id
left join [user] super
on super_ce.ce_supervisor_id = super.user_id
...
BEGIN
    ...
    -- DECLARE @C_COL VARCHAR(255) = (SELECT 'COMMENTS_' + CAST(@QUESTION_NUMBER as VARCHAR))
    -- IF COL_LENGTH('#tmp_results', @C_COL) IS NULL
    -- BEGIN
    --     EXEC('ALTER TABLE #tmp_results ADD [' + @C_COL+ '] VARCHAR(255)')
    -- END
...
BEGIN
    ...
    begin
        ...
        -- set @SQL = ''
        -- select @SQL =  @SQL + 'update #tmp_results set COMMENTS_' + CAST(question_number as VARCHAR) + ' = ''' + comments + ''' where review_id = ' + cast(@REVIEW_ID as VARCHAR(255))
        -- from #tmp_questions_and_answers
        -- exec(@SQL)
